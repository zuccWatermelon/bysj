          ...
          <div class="login-main">
             ...
             <div class="login-cloud"  v-if="login">
                 <div class="login-fold">
                   ...
                   <p><span>使用费 : {{message.conf_price}}元</span><span>使用费折扣 : <span class="add-color">{{discountMsg}}</span></span><span>折扣后费用 : <span class="add-color">{{parseFloat(message.conf_price)*parseFloat(message.discount)}}元</span></span></p>
                   <p><span>一次性费用 : {{message.calculate_amount}}元</span><span>一次性费用实收：{{message.onceChargePriceAfterDiscount}}元</span><span>折后总价 : <span class="add-color">{{parseFloat(message.conf_price)*parseFloat(message.discount)+parseFloat(message.onceChargePriceAfterDiscount)}}元</span></span></p>
                 </div>
...
<script>
  export default {
    ...
    methods:{
      close(){
        let _this = this;
        _this.$emit('update:loginClick', false);
      },
      //审批提交
      submitToExamine(){
        let _this = this;
         ...
         axios({
            ...
            params: {
              params: {
                ...
                "catalogId":_this.message.catalogId.toString(),
                "bandwidthItem": {
                  ...
                  "offerAt": _this.message.conf_price,                    //订购商品原价
                  "offerRealAt": parseFloat(_this.message.conf_price) * parseFloat(_this.message.discount),//商品折扣后价格
                  "offerRate": _this.message.discount,             //订购商品折扣
                  "offerId":_this.message.offerId,
                  "workAt": _this.message.calculate_amount,              //初装费原价
                  "workProdId":'222',
                  "workRealAt":_this.message.onceChargePriceAfterDiscount,    //初装费折后价格
                  "workRate":_this.message.workRate,                              //初装费折扣
                  "billType":_this.message.billType,  //付费方式
                }
              }
            }
          }).then(responce => {
    ...
    mounted(){
        ...
        if(self.message.discount == '0'){
          self.discountMsg = '减免';
        }else if(self.message.discount == '0.05'){
          self.discountMsg = '0.5折';
        }else if(self.message.discount.length == 4){
          self.discountMsg = parseFloat(self.message.discount * 100).toFixed(0) + "折";
        }else{
          self.discountMsg = parseFloat(self.message.discount * 10).toFixed(0) + "折";
        }
