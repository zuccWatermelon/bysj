      ...
      <div class="login-main">
        ...
        <div class="login-cloud"  v-if="login">
          <div class="login-fold">
            ...
            <p><span>使用费 : {{message.amount}}元</span><span>使用费折扣：<span class="add-color">{{discountMsg}}</span></span><span>折扣后费用 : <span class="add-color" style="width:100px;">{{parseFloat(message.amount * message.discount).toFixed(2)}}元</span></span></p>
          </div>
  ...
  export default {
    ...
    created(){
      ...
      if(self.message.discount == "0"){
        ...
        self.discountMsg = parseInt(self.message.discount * 100).toString() + '折';
      }else if (self.message.discount.length == 3){
        self.discountMsg = parseInt(self.message.discount * 10).toString() + '折';
      }
    },
  }
</script>
